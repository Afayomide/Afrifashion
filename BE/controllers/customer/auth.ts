...
const signToken = (id: string | Types.ObjectId): string => {
...
export const checkAuth = async (req: Request, res: Response) => {
  try {
    const user = await Customer.findById((req as unknown as AuthRequest).user.userId).select("-password"); // Exclude password field
    if (!user) {
...
export const resetPassword = async (req: Request, res: Response) => {
  ...
  const jwtToken = signToken(user._id as Types.ObjectId);
