...
export const pay = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : "An error occurred while initializing payment";
    console.error(error)
    res.status(500).json({
      status: "error",
...
export const verifyPayment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (status && data.status === "success") {
      ...
      const products = productData.map((data: IProductData) => ({
    ...
    const errorMessage = error instanceof Error ? error.message : "An error occurred while verifying payment";
    console.error(error);
    res.status(500).json({
      status: "error",
