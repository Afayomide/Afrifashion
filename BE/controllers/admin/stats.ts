...
export const updateCustomerStats = async (
  ...
  try {
    ...
    // Get new customers
    const newCustomersDaily = await Customer.countDocuments({
      role: "customer",
      createdAt: { $gte: today },
    });
    const newCustomersWeekly = await Customer.countDocuments({
      role: "customer",
      createdAt: { $gte: oneWeekAgo },
    });
    const newCustomersMonthly = await Customer.countDocuments({
      role: "customer",
      createdAt: { $gte: oneMonthAgo },
    });
    const newCustomersYearly = await Customer.countDocuments({
      role: "customer",
      createdAt: { $gte: oneYearAgo },
    });
